...
class MFModel(nn.Module):
    ...
    def predict(self, user_ids):
        ...
        scores = torch.matmul(u, all_items.T)  # (batch, n_items)
        return scores


def bpr_loss(pos_score, neg_score, reg=1e-5):
...
def train_mf(train_df, embed_dim=64, lr=1e-3, reg=1e-5, epochs=50,
    ...
    # Map IDs to indices in dataframe
    ...
    for epoch in range(epochs):
        ...
        for users, pos_items, neg_items in loader:
            ...
            loss = bpr_loss(pos_score, neg_score, reg)
...
def run_mf(train_path="data/processed/train.csv", test_path="data/processed/test.csv",
           output_path="results/mf_scores.csv", embed_dim=64, lr=1e-3, epochs=50, k=10):
    """Run full MF pipeline."""
    ...
    model, user2idx, movie2idx, idx2movie = train_mf(
        train_df, embed_dim=embed_dim, lr=lr, epochs=epochs, device=device
    )
    ...
    # Build train user items (to exclude from predictions)
        ...
        for start in range(0, len(test_users), batch_size):
            ...
            for i, uid in enumerate(batch_users):
                ...
                for movie_id, score in ranked:
                    all_scores.append({"user_id": uid, "movie_id": movie_id, "mf_score": score})
